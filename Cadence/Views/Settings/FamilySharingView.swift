...
struct FamilySharingView: View {
    ...
    var body: some View {
        ...
        .alert("Enable iCloud Sync", isPresented: $showEnableAlert) {
            ...
            Text("This will enable iCloud sync. Events marked as \"Shared\" will be visible to family members. You can choose which events to share.")
        }
        .alert("Disable iCloud Sync", isPresented: $showDisableAlert) {
            Button("Turn Off", role: .destructive) {
                iCloudSyncEnabled = false
                syncService.syncStatus = .disabled
            }
            Button("Cancel", role: .cancel) {}
        } message: {
            Text("Shared events will remain on this device but stop syncing. Other family members will no longer see updates.")
        }
    }
    ...
    private var syncToggleSection: some View {
        Section {
            ...
            Text("Requires an active iCloud account signed in on this device.")
                .font(.caption)
    ...
    private var setupInstructionsSection: some View {
        Section("How to Set Up Family Sharing") {
            VStack(alignment: .leading, spacing: CadenceTheme.spacingSM) {
                ...
                instructionRow(number: 4, text: "Enable iCloud Sync above, then mark events as \"Shared\" to make them visible to everyone.")
            }
            .padding(.vertical, CadenceTheme.spacingXS)
            .listRowBackground(CadenceTheme.backgroundSecondary)
        }
